...
const CreditCard = ({ courseName, courseImg, title }) => {
  // Split courseName by commas and trim whitespace
  const tags = courseName.split(",").map((tag) => tag.trim());

  return (
    <Card
      shineStrength={0.1}
      className="cursor-pointer rounded-lg group shadow-lg overflow-hidden bg-gradient-to-r from-gray-700 to-gray-700 p-4 flex flex-col items-center text-white"
      onClick={() => console.log("Hola")}
    >
      <div className="group   rounded-md overflow-hidden transition-all duration-200 max-w-[328px] sm:w-full w-[328px] hover:border-[#3D3D3D] lg:p-6 md:p-5 p-4 relative flex flex-row">
        <div className="flex flex-col md:w-[232px] w-[226px]">
          ...
          <div className="flex flex-row items-start lg:pt-4 pt-3 flex-wrap md:w-[232px] w-[220px] overflow-y-auto">
            {tags.map((tag, index) => (
              <a
                key={index}
                className="md:text-[14px] text-[12px] md:mr-2 mr-1 leading-[20px] my-1 lg:px-5 px-3 md:py-2 py-1 text-[#3D3D3D] text-center border border-[#D9DCE1] rounded-full cursor-pointer"
              >
                {tag}
              </a>
            ))}
        ...
        <div className="md:w-[106px] flex items-center w-[56px]">
          <div className="absolute aspect-square flex items-center justify-left rounded-full bg-secondary h-[100%] md:h-[125%]">
            <img
              ...
              width="40"
              height="40"
              className="w-10  h-10 md:w-[46px] md:h-[46px] xl:w-[72px] xl:h-[72px] bg-center bg-cover bg-no-repeat md:ml-6 xl:ml-1 ml-6 transition-transform duration-300 group-hover:scale-110"
              src={courseImg}
            />
          </div>
        </div>
